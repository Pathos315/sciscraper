...
class JSONScrape(ScrapeRequest, slookup_code='json'):
    ...
    def download(self, search_text:str) -> dict:
        ...
        print(f'[sciscraper]: Searching for {search_text} via a {self.search_field}-style search.', 
            end = '\r')
        querystring = {'search_mode':'content',
        ...
        for item in self.docs:
            self.data = self.get_data_entry(item, keys=
                ['title','author_list','publisher',
                'pub_date','doi','id','abstract',
                'acknowledgements','journal_title','volume',
                'issue','times_cited','mesh_terms', 'cited_dimensions_ids'])
        return self.data
...
class PDFScrape:
    ...
    def download(self, search_text:str) -> dict:
        ...
        with pdfplumber.open(self.search_text) as self.study:
            ...
            self.postprints = [re.sub(r'\W+', ' ', manuscript) for manuscript in self.manuscripts] 
    ...
    def get_target_words(self):
        ...
        self.target_words = ["prosocial", "design", "intervention",
                        "reddit", "humane","social media",
                        "user experience","nudge","choice architecture",
                        "user interface", "misinformation",
                        "disinformation", "Trump", "conspiracy",
                        "dysinformation", "users","Thaler","Sunstein","boost"]
    ...
    def get_bycatch_words(self):
        ...
        '''
        self.bycatch_words = ["psychology", "pediatric", "pediatry", 
                            "autism", "mental", "medical",
                            "oxytocin", "adolescence", "infant", "health", 
                            "wellness", "child", "care", "mindfulness"]
    ...
    def get_research_words(self):
        ...
        self.research_words = ["big data", "data", "analytics", "randomized controlled trial", 
                            "RCT", "moderation", "community", "social media", "conversational", 
                            "control", "randomized", "systemic", "analysis", "thematic", "review", 
                            "study", "case series", "case report", "double blind", 
                            "ecological", "survey"]
...
class PubIDRequest(FileRequest, dlookup_code = 'pub'):
    ...
    def fetch_terms(self):
        ...
        self.search_terms = (search_text for search_text in \
            self.df['cited_dimensions_ids'] if search_text is not None)
        self.src_title = pd.Series(self.df['title'])

        return pd.DataFrame([self.scraper.download(search_text) for \
            search_text in tqdm(list(self.search_terms))]).join(self.src_title)
...
class FolderRequest(FileRequest, dlookup_code='fold'):
    ...
    def __init__(self, target: PathLike[str], slookup_key:bool = False):
        ...
        self.target = target
        if self.slookup_key:
            raise Exception("This action is prohibited. \
                You already have the files that this query would return.")
        self.slookup_key = slookup_key
        self.scraper = PDFScrape()

    def fetch_terms(self):
        self.search_terms = [path.join(self.target, file) for file in \
            listdir(self.target) if fnmatch(path.basename(file), '*.pdf')]
        return pd.DataFrame([self.scraper.download(file) for \
            file in tqdm(self.search_terms)])

...
class RelevanceCalculator:
    ...
    def __call__(self) -> float:
        ...
        try:
            wordscore = self.calculate_final_score(
                pos_odds,
                neg_odds,
                len_factor,
    ...
    def calculate_final_score(
        ...
        raw_score: float = pos_odds / (pos_odds + neg_odds)  # Calculate the raw score
        adjusted_score = (
            raw_score * len_factor
        )  # Adjust the score based on the length of the word
        return max(0.00, adjusted_score)
    ...
    def get_len_factor(self) -> float:
        ...
        adjusted_probability: float = float(
            np.mean(
                [
                    probability,

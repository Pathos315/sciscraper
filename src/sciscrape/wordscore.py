...
class RelevanceCalculator:
    ...
    Attributes:
        pos_part (float): The number of times a target word appeared in the paper or abstract.
        neg_part (float): The number of times a bycatch word appeared in the paper or abstract.
        total_len (int): The total number of words in the paper or abstract.
        implicature_score (float, Optional): The highest score from a prior `DocScraper` zero-shot classification.
    ...
    def __call__(self) -> float:
        """
        Calculates the relevance of the paper, or abstract, as a percentage, by using a binomial distribution formula.

        Returns:
            float: The probability that the paper is relevant, as a percentage.

        Example:
        >>> calculator = RelevanceCalculator(pos_part=2, neg_part=1, total_len=100, implicature_score=0.9)
        >>> calculator()
        0.815 # or 81.5%
        """
        ...
        choice: int = comb(weighted_length, self.pos_part)
        probability: float = self.get_probability(self.pos_part, weighted_length)
        failure_prob: float = self.get_probability(neutral_part, weighted_length)
    ...
    def calculate_wordscore(self, raw_probability: float) -> float:
        """
        Calculates the final word score based on the raw probability and the implicature score.
    ...
    @staticmethod
    def get_probability(part: Union[int, float], whole: Union[int, float]) -> float:
        """

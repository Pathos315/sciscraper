...
def unpack_txt_files(txtfile: str, __encoding="utf8") -> set[str]:
    """unpacks_txt_files takes a txt_file containing indended words and relays them to the PDFScraper class.

    Args:
        wordset ([type]): [description]
        __encoding (str, optional): [description]. Defaults to "utf8".
...
class PDFScraper(Scraper):
    def __init__(self, research_words: str, bycatch_words: str, target_words: str):
        self.research_words = unpack_txt_files(research_words)
        self.bycatch_words = unpack_txt_files(bycatch_words)
        self.target_words = unpack_txt_files(target_words)

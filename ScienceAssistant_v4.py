import os, pdfplumber, datetime, time, threading
...
def get_user_input():
    ...
    file = input("Input first file # to import from: ")
    num_files = input("Input total # of files to import: ")
    
    params = [file, num_files]
    ...
    if all(str(i).isdigit() for i in params):  # Check input is valid
        ...
        print(
            ...
            "\nFile index 0, all files in folder.",
        )
        params = [0, 0]
...
def main(folder, file, num_of_files): #directory = dir, file = starting at, num_of_file = ending at
    ...
    files, num_files = get_user_input()
    ...
    process_loop(num_files, full_dir, folder)
    
def process_loop(num_files, full_dir, folder):
    ...
    for file in range(num_of_files):
        if file <= num_files:
            try:
                ...
                extract(n, study, filename, file, num_files, filepath)
...
def extract(n, study, filename, file, num_files, filepath):
    ...
    for page in range(n):
        if page <= n:
            ...
            print(f"Processing file {file} of {num_files} | {filepath} | Page {page} of {n}...")
...
def overlap(li1,li2): #Possible lambda function?
    li3 = [w for w in li1 if w in li2]
    return li3

def score(target_1, target_2): #Possible lambda function?
...
compendium = []
main(dir, 0, 0) #dir = a pathname to a folder that contains research papers

#Could the below become a def in a class?
...
df_new.to_csv(f'{csv_date}_studies_test.csv')
